...
export default function Results() {
  ...
  const getBannerStyles = (color: string) => {
      switch(color) {
          case 'red': return "bg-red-100 border-red-500 text-red-800 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800";
          case 'yellow': return "bg-yellow-100 border-yellow-500 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 dark:border-yellow-800";
          default: return "bg-green-100 border-green-500 text-green-800 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800";
      }
  };

  const getTestRowStyles = (status: string) => {
      const s = status.toLowerCase();
      if (s === 'high' || s.includes('risk')) return "bg-red-50/50 text-red-900 font-medium dark:bg-red-900/20 dark:text-red-200";
      if (s === 'borderline') return "bg-yellow-50/50 text-yellow-900 dark:bg-yellow-900/20 dark:text-yellow-200";
      return "";
  };
      ...
      <div className="container py-8 md:py-12 print:p-0">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            <Card className={`border-2 ${overallRisk === 'High' ? 'border-red-500/50 bg-red-50/10 dark:bg-red-950/20' : 'border-primary/20 bg-primary/5 dark:bg-primary/10'}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                  <Stethoscope className={`h-5 w-5 ${overallRisk === 'High' ? 'text-red-600 dark:text-red-400' : 'text-primary'}`} />
                  Recommended Specialist
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                 <div className="rounded-lg bg-card p-4 shadow-sm border border-border/50">
                  <p className="text-sm font-medium text-muted-foreground">Based on severity:</p>
                  <p className={`text-xl font-bold ${overallRisk === 'High' ? 'text-red-600 dark:text-red-400' : 'text-primary'}`}>
        ...
        <div className="mt-8 rounded-xl border border-border bg-secondary/30 dark:bg-secondary/10 p-6">
